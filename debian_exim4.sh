...
#DEPLOY_EXIM4_CONF="/etc/exim4/exim4.conf.localmacros"
...
debian_exim4_deploy() {
  ...
  _ssl_path="/etc/exim4/certs/$_cdomain"
  ...
  _real_fullchain="$_ssl_path/fullchain.pem"
  ...
  # chown&chmod
  chown root:Debian-exim "$_ssl_path"
  chown root:Debian-exim "$_ssl_path/key.pem"
  chown root:Debian-exim "$_ssl_path/fullchain.pem"

  chmod 750 "$_ssl_path"
  chmod 640 "$_ssl_path/key.pem"
  chmod 640 "$_ssl_path/fullchain.pem"

  DEFAULT_EXIM4_RELOAD="service exim4 restart"
  _reload="${DEPLOY_EXIM4_RELOAD:-$DEFAULT_EXIM4_RELOAD}"

  if [ -z "$IS_RENEW" ]; then
    DEFAULT_EXIM4_CONF="/etc/exim4/exim4.conf.localmacros"
    ...
    _backup_conf="$DOMAIN_BACKUP_PATH/exim4.conf.localmacros.bak"
    _info "Backup $_exim4_conf to $_backup_conf"
    cp "$_exim4_conf" "$_backup_conf"

    _info "Modify exim4 conf: $_exim4_conf"
    if _setopt "$_exim4_conf" "MAIN_TLS_ENABLE" "=" "yes" &&
      _setopt "$_exim4_conf" "MAIN_TLS_CERTIFICATE" "=" "$_real_fullchain" &&
      _setopt "$_exim4_conf" "MAIN_TLS_PRIVATEKEY" "=" "$_real_key"; then
